...
export function SensitivityPanel() {
  ...
  const add = useStore((s) => s.addSensitivityRow);
  const update = useStore((s) => s.updateSensitivityRow);
  const remove = useStore((s) => s.removeSensitivityRow);
  ...
  return (
    <div className="check-panel">
      ...
      {calc.sensitivity.length === 0 && (
        <div className="empty-hint">Нет строк чувствительности.</div>
      )}
